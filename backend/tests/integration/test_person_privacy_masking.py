"""Real-database integration tests for living-member privacy masking.

View-only viewers (VIEWER-role tree members, and anonymous public
share-link visitors) must never see a *living* person's birth/death dates,
birth/death locations, or Notes ("More details") — only full name, Sex, and
Status. Deceased persons are unaffected: their full details remain visible
to view-only viewers, same as today.

Follows the same real-Postgres pattern as test_relationships.py (see that
module's docstring for local setup instructions). Requires
...
if not TEST_DATABASE_URL:
    pytest.skip(
        ...
        "database (see test_relationships.py for local setup). Skipping.",
...
class Seed:
    ...
    deceased person (Bob), both with full "more details" + notes set."""
...
class TestGetPersonMasksLivingMembersForViewers:
    @pytest.mark.asyncio
    async def test_viewer_does_not_see_living_persons_dates_or_notes(self, seed: Seed):
        from src.api.v1.persons import get_person

        result = await get_person(seed.tree_id, seed.alice_id, seed.viewer, seed.session)

        assert result.display_given_name == "Alice"
        assert result.sex == "FEMALE"
        assert result.is_living is True
        assert result.birth_date is None
        assert result.birth_year is None
        assert result.born_city is None
        assert result.born_country is None
    ...
    async def test_viewer_sees_deceased_persons_full_details(self, seed: Seed):
        ...
        result = await get_person(seed.tree_id, seed.bob_id, seed.viewer, seed.session)

        assert result.display_given_name == "Bob"
        assert result.is_living is False
        assert result.birth_date == date(1920, 3, 1)
        assert result.death_date == date(1990, 12, 1)
        assert result.born_city == "Metropolis"
        assert result.died_city == "Gotham"
        assert result.notes == "War veteran"

    @pytest.mark.asyncio
    async def test_owner_sees_living_persons_full_details(self, seed: Seed):
        from src.api.v1.persons import get_person

        result = await get_person(seed.tree_id, seed.alice_id, seed.owner, seed.session)

        assert result.birth_date == date(1990, 5, 1)
        assert result.born_city == "Springfield"
        assert result.notes == "Loves gardening"
